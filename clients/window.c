...
tablet_set_cursor_image_index(struct tablet *tablet, int index)
{
	...
	cursor = tablet->display->cursors[tablet->current_cursor];
			    ...
			    uint32_t time)
{
	...
	if (widget->tablet_proximity_out_handler)
...
static void
tablet_handle_down(void *data, struct wl_tablet *wl_tablet, uint32_t time)
{
	struct tablet *tablet = data;
	struct widget *focus = tablet->focus_widget;
